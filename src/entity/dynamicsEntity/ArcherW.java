 ...
 */
public class ArcherW extends Player{
  
  private Arrow arrow;
  private int  atk=0;

  //COSTRUTTORE
  public ArcherW(Handler handler,float x, float y) {   
    ...
    anim= new PlayerAnimation(handler,60,120,100,Assets.ArcherWomananimation,Assets.ArcherWoman_animationAtk, Assets.ArcherWoman_animationDeath);
  }
  ...
  protected void Attack() {
    if (handler.getMouseManager().isLeftPressed() && !dead){
           ...
           switch (anim.getIndex()) {
             ...
             if(!handler.getMouseManager().isLeftPressed()){
                    atk=0;
                 //   handler.getWorld().getEntityManager().removeEntity(arrow);
             }
  }
}
