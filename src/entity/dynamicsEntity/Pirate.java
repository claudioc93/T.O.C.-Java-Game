...
import gfx.Assets;
import gfx.animation.PirateAnimation;
 ...
 */
public class Pirate extends Creatures {
  ...
  public Pirate(Handler handler, float x, float y) {
    ...
    anim= new PirateAnimation(handler,60,180,100,Assets.pirateanimation,Assets.pirate_animationAtk, Assets.pirate_animationDeath); 
    }         
    ...
    if(!handler.getWorld().getEntityManager().getPlayer().dead){
      ...
      for (Entity e: handler.getWorld().getEntityManager().getEntities()){
        if (!e.equals(handler.getWorld().getEntityManager().getPlayer()))
         continue;
             if (e.getCollisionBounds(0, 0).intersects (ar)&& timerAtk-lastAtk>200){
